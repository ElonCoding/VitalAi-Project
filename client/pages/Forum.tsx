...
export default function Forum() {
  ...
  const [isModerator, setIsModerator] = useState<boolean>(
    () => sessionStorage.getItem("dpis_mod") === "1",
  );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("forum:updated", h as EventListener);
  }, []);
  ...
  const languages = useMemo(
    () => Array.from(new Set(posts.map((p) => p.language || "English"))),
    [posts],
  );
  const tags = useMemo(() => {
  ...
  function toggleModerator() {
    ...
    setIsModerator(next);
    try {
      sessionStorage.setItem("dpis_mod", next ? "1" : "0");
    } catch {}
  }
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                A moderated, anonymous space where students can share
                experiences and support each other.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <Button size="sm" variant="ghost" onClick={toggleModerator}>
                {isModerator ? "Moderator: ON" : "Moderator: OFF"}
              </Button>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="mt-4">
                <div className="text-xs text-muted-foreground">
                  Community rules: Be respectful, no hate speech, no doxxing.
                  Posts mentioning self-harm are flagged and routed to
                  helplines.
                </div>
              </div>
            </div>
