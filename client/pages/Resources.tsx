...
const SAMPLE: Resource[] = [
  {
    ...
    description: "A short guided relaxation to reduce acute stress and ground attention.",
  },
  {
    id: "r2",
    ...
    description: "Demonstration of paced breathing (4‑7‑8) with calming visuals.",
  },
  {
    id: "r3",
  ...
  },
  {
    ...
    description: "Evidence-based sleep hygiene practices to improve sleep quality.",
  },
  {
    id: "r5",
...
export default function Resources() {
  ...
  const languages = useMemo(() => Array.from(new Set(SAMPLE.map((r) => r.language))), []);
  const tags = useMemo(() => {
  ...
  const filtered = SAMPLE.filter((r) => {
    ...
    if (query && !(`${r.title} ${r.tags.join(" ")} ${r.description || ""}`.toLowerCase().includes(query.toLowerCase()))) return false;
    return true;
  });
    ...
    <main className="relative">
      ...
      <section className="container py-12">
        <div className="mx-auto max-w-4xl">
          <h1 className="text-3xl font-semibold">Psychoeducational Resource Hub</h1>
          <p className="mt-2 text-sm text-muted-foreground">Multimedia library with videos, relaxation audio, and step-by-step wellness guides in multiple languages.</p>

          <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              <input placeholder="Search resources" value={query} onChange={(e) => setQuery(e.target.value)} className="rounded-md border bg-background px-3 py-2" />
              <Button variant="outline" size="sm" onClick={() => { setQuery(""); setLanguage("All"); setCategory("All"); }}>Reset</Button>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            {filtered.length === 0 && <div className="text-sm text-muted-foreground">No resources found.</div>}
          </div>
        </div>
      </section>
    </main>
